...
class CLWDDataset(torch.utils.data.Dataset):
    def __init__(self, is_train, args):
        ...
        if args.is_train == True:
            print("TRAIN")
            self.root_main = args.dataset_dir + '/train/'
            self.root_sec = self.root_main
            # self.keep_background_prob = 0.01
            self.keep_background_prob = -1
        elif args.is_train == False:
            self.root_main = args.dataset_dir + '/test/' #'/test/'
            self.root_sec = args.dataset_dir + '/train/' #'/test/'
            self.keep_background_prob = -1
        ...
        self.imageJ_path=osp.join(self.root_main,'Watermarked_image','%s.jpg')
        self.imageI_path=osp.join(self.root_main,'Watermark_free_image','%s.jpg')
        self.mask_path=osp.join(self.root_main,'Mask','%s.png')
		# self.balance_path=osp.join(self.root,'Loss_balance','%s.png')
        self.alpha_path=osp.join(self.root_sec,'Alpha','%s.png')
        self.W_path=osp.join(self.root_sec,'Watermark','%s.png')
