"""
Eval the effect of filling prior to generation.
"""
...
def main(args):
    ...
    for batch in tqdm.tqdm(dataloader):
        ...
        if "morphomod" in args.model_type:
            imfinal, immask, raw_out, fill_x = model.forward_w_gt(wm, mask, args.dilate, args.prompt, fill=args.fill)
        else:
    ...
    # save everything
    with open(os.path.join(args.expr_path, f'{args.dataset}_{args.fill}_metrics.json'), 'w') as file:
