"""
Eval the effect of different prompts.
...
def main(args):
    ...
    for folder in ["wm", "mask", "gen_mask", "imfinal", "raw_out"]:
    ...
    for batch in tqdm.tqdm(dataloader):
        ...
        if "morphomod" in args.model_type:
            imfinal, immask, raw_out = model.forward_w_gt(wm, mask, args.dilate, args.prompt)
        else:
        ...
        #
        if cnt < 150:
            for im, m, gen_m, fin, raw in zip(wm, mask, immask, imfinal, raw_out):
